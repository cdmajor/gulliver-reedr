...
const BAKED_API_URL = "%%VICTOR_API_URL%%";
...
function isConfiguredUrl(url) {
  ...
  if (!trimmed || trimmed.includes("%%") || trimmed.includes("VICTOR_API_URL")) return false;
  try {
...
async function resolveApiUrl() {
  try {
    const result = await browserAPI.storage.sync.get(["victorApiUrl"]);
    if (isConfiguredUrl(result.victorApiUrl)) {
      return result.victorApiUrl.trim().replace(/\/$/, "");
    }
  } catch (_) {
...
browserAPI.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === "VICTOR_EXTRACT_PDF") {
...
browserAPI.runtime.onConnect.addListener((port) => {
  if (port.name !== "victor-chat") return;
  ...
  port.onMessage.addListener(async (msg) => {
    ...
    try {
      ...
      if (!apiUrl) {
        port.postMessage({ type: "ERROR", error: "Victor is not configured. Open Settings to set your API URL." });
        return;
      }
      const endpoint = apiUrl + "/victor/chat";
...
async function handleChat({ messages, pageContext }) {
  const apiUrl = await resolveApiUrl();
  if (!apiUrl) return { error: "Victor is not configured. Open Settings to set your API URL." };
  const endpoint = apiUrl + "/victor/chat";
  try {
    const response = await fetch(endpoint, {
      method: "POST",
...
async function handleExtractPdf({ pdfUrl }) {
  const apiUrl = await resolveApiUrl();
  if (!apiUrl) return { error: "Victor is not configured." };
  const endpoint = apiUrl + "/victor/extract-pdf";
  try {
    const response = await fetch(endpoint, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
