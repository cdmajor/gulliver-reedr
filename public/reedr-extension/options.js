...
browserAPI.storage.sync.get(["reedrApiUrl"]).then((result) => {
...
saveBtn.addEventListener("click", () => {
  ...
  browserAPI.storage.sync.set({ reedrApiUrl: url }).then(() => {
    ...
    setEffective(url);
    showStatus("Settings saved! Reedr is ready.", "success");
  }).catch(() => {
    saveBtn.disabled = false;
    saveBtn.textContent = "Save Settings";
...
testBtn.addEventListener("click", async () => {
  ...
  testBtn.textContent = "…";
  try {
    // Prefer a lightweight Reedr endpoint; /health may not exist on all hosts
    const candidates = [url + "/reedr/extension-download?origin=https://example.com", url + "/health", url];
    let ok = false;
    let lastStatus = 0;
